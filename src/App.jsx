...
const CUR = {
  ...
  bossShards: { name: "Boss Shards", icon: "◈", color: "#cc3333", rarity: "PERMANENT", desc: "Crystallised power from bosses. Unlocks custom ship designs." },
};
...
export default function VoidStorm(){
  ...
  const[showPauseSettings,setShowPauseSettings]=useState(false);const[showPauseLabs,setShowPauseLabs]=useState(false);
  ...
  const[syncCode,setSyncCode]=useState(null);const[syncStatus,setSyncStatus]=useState("none");const[syncCodeInput,setSyncCodeInput]=useState("");const[showShipPopup,setShowShipPopup]=useState(false);const[showBulletPopup,setShowBulletPopup]=useState(false);const[showBgPopup,setShowBgPopup]=useState(false);const[showSyncInfo,setShowSyncInfo]=useState(false);const[syncConflict,setSyncConflict]=useState(null);const[showDesignPopup,setShowDesignPopup]=useState(false);const[showChangelog,setShowChangelog]=useState(false);const[clExpanded,setClExpanded]=useState({});const syncCodeRef=useRef(null);const _cloudDebounce=useRef(null);const _sessionIdRef=useRef(Math.random().toString(36).slice(2,10)+Date.now().toString(36));const[sessionTakeover,setSessionTakeover]=useState(false);const[confirmDisconnect,setConfirmDisconnect]=useState(false);
  ...
  const _cloudPush=useCallback((ms)=>{
    ...
    fetch(SUPABASE_URL+"/rest/v1/saves",{method:"POST",headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify({code,data:{meta:ms,history:JSON.parse(h),tut:t,session_id:_sessionIdRef.current},updated_at:new Date().toISOString()})}).catch(()=>{});}catch(e){}
  },[]);
  const _cloudPull=useCallback(()=>{
    const code=syncCodeRef.current;if(!code||!_SYNC_OK)return;
    fetch(SUPABASE_URL+"/rest/v1/saves?code=eq."+code+"&select=data",{headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY}}).then(r=>r.json()).then(rows=>{if(rows.length>0&&rows[0].data){const cd=rows[0].data;if(cd.session_id&&cd.session_id!==_sessionIdRef.current){setSessionTakeover(true);return;}if(cd.meta){const cloudAt=cd.meta.savedAt||0;const localAt=metaRef.current.savedAt||0;if(cloudAt>localAt){setMeta(cd.meta);try{localStorage.setItem("vs4-meta",JSON.stringify(cd.meta));}catch(e){}if(cd.history)try{localStorage.setItem("vs4-history",JSON.stringify(cd.history));}catch(e){}if(cd.tut)try{localStorage.setItem("vs4-tut",cd.tut);}catch(e){}}}}}).catch(()=>{});
  },[]);
  ...
  useEffect(()=>{syncCodeRef.current=syncCode;if(syncCode==="9292"){setMeta(prev=>{const nx={...prev};/* Max all labs */const comp={};LAB_UPGRADES.forEach(lu=>{comp[lu.id]=lu.levels.length;});nx.lab={...(nx.lab||{}),slots:3,completed:comp,active:nx.lab?.active||[],totalWaves:nx.lab?.totalWaves||0,saved:nx.lab?.saved||[]};/* Unlock all designs */nx.ownedDesigns=SHIP_DESIGNS.filter(d=>d.cost>0).map(d=>d.id);nx.bossShards=Math.max(nx.bossShards||0,500);saveMeta(nx);return nx;});}},[syncCode]);
  useEffect(()=>{
    if(!_SYNC_OK)return;
  ...
  useEffect(()=>{
    ...
    const id=setInterval(()=>{_cloudPull();},30000);
  ...
  function initTutorial(){
    ...
    const gs={
      ...
      wave:0,waveActive:false,enemiesLeft:0,waveTotal:0,waveKilled:0,
      spawnQueue:[],spawnTimer:0,kills:0,scrap:0,cores:0,plasma:0,
      upgrades:{},usedAbIds:[],deathCause:"",newEnemyNotif:null,_labNotifs:[],dmgTrack:{},waveDmg:{},_diffuseCount:0,_diffuseMult:1,_sprintLabWaves:0,
  ...
  function offerAb(gs){
    ...
    const _dcLvl3=metaRef.current.lab?.completed?.diffusion_chance||0;const _dcPct3=_dcLvl3>0?_dcLvl3*10:0;const _dmLvl3=metaRef.current.lab?.completed?.diffusion_multi||0;const _dmBonus3=0.0375+_dmLvl3*0.009;gs._showDiffuse=!gs.isTutorial&&!gs.isNewMode&&!gs.isPlayground&&(metaRef.current.metaTier||1)>=2&&_dcPct3>0&&Math.random()*100<_dcPct3;gs._diffuseBonus=_dmBonus3;setAbChoices(picks);setPhase("ability");
  }
  ...
  function fireEB(gs,e){
    ...
    switch(e.pattern){
      ...
      case"enf_wraith":{/* Phase assault - multi-angle spreads + orbiting shots + rapid aimed */const _wp=Math.floor(gs.time/250)%4;const _wOffsets=[0,PI2/4,PI2/2,-PI2/4];const _wBase=_wOffsets[_wp];for(let i=-3;i<=3;i++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+_wBase+i*0.12)*bs*1.3,vy:Math.sin(a+_wBase+i*0.12)*bs*1.3,size:5,dmg:dmg*0.55});}/* orbiting shots */const _wo=gs.time*0.007;for(let i=0;i<5;i++){const _oa=_wo+i*(PI2/5);gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(_oa)*bs*0.8,vy:Math.sin(_oa)*bs*0.8,size:4,dmg:dmg*0.4});}/* dual fast aimed */for(let i=-1;i<=1;i+=2){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+i*0.08)*bs*2.2,vy:Math.sin(a+i*0.08)*bs*2.2,size:3,dmg:dmg*0.85});}break;}
      case"enf_fortress":{/* Siege mode - rotating shield wall of slow bullets + aimed artillery */const _fRot=gs.time*0.002;/* outer wall ring rotating slowly */const _fN=14;for(let i=0;i<_fN;i++){const ra=(PI2/_fN)*i+_fRot;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra)*bs*0.35,vy:Math.sin(ra)*bs*0.35,size:6,dmg:dmg*0.3});}/* inner fast ring, opposite rotation */const _fN2=10;for(let i=0;i<_fN2;i++){const ra2=(PI2/_fN2)*i-_fRot*1.5;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra2)*bs*0.8,vy:Math.sin(ra2)*bs*0.8,size:4,dmg:dmg*0.5});}/* heavy aimed artillery */for(let i=-1;i<=1;i++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+i*0.15)*bs*1.5,vy:Math.sin(a+i*0.15)*bs*1.5,size:7,dmg:dmg*0.8});}break;}
      case"enf_reaper":{/* Death spiral - rotating triple arms + fast aimed snipes */const _rsOff=gs.time*0.003;for(let arm=0;arm<3;arm++){const armA=_rsOff+arm*(PI2/3);for(let j=0;j<5;j++){const ba=armA+j*0.25;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ba)*bs*(0.5+j*0.2),vy:Math.sin(ba)*bs*(0.5+j*0.2),size:5,dmg:dmg*0.4});}}for(let i=-1;i<=1;i++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+i*0.1)*bs*2.0,vy:Math.sin(a+i*0.1)*bs*2.0,size:4,dmg:dmg*0.8});}break;}
      case"enf_boss":{/* Void commander - cycles expanding ring + aimed fan + converging walls */const _bp=Math.floor(gs.time/500)%4;if(_bp===0){const n=16;for(let i=0;i<n;i++){const ra=(PI2/n)*i+gs.time*0.002;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra)*bs*0.6,vy:Math.sin(ra)*bs*0.6,size:5,dmg:dmg*0.4});}for(let i=0;i<8;i++){const ra2=(PI2/8)*i+gs.time*0.002+0.4;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra2)*bs*1.1,vy:Math.sin(ra2)*bs*1.1,size:3,dmg:dmg*0.3});}}else if(_bp===1){for(let i=-4;i<=4;i++)gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+i*0.1)*bs*1.3,vy:Math.sin(a+i*0.1)*bs*1.3,size:5,dmg:dmg*0.7});}else if(_bp===2){/* converging walls from sides */for(let i=0;i<6;i++){gs.eBullets.push({x:10,y:rand(50,GH-50),vx:bs*0.8,vy:rand(-0.3,0.3)*bs,size:5,dmg:dmg*0.5});gs.eBullets.push({x:GW-10,y:rand(50,GH-50),vx:-bs*0.8,vy:rand(-0.3,0.3)*bs,size:5,dmg:dmg*0.5});}gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*bs*1.8,vy:Math.sin(a)*bs*1.8,size:4,dmg:dmg*0.9});}else{/* rotating laser arms */const _la=gs.time*0.004;for(let arm=0;arm<4;arm++){const armA=_la+arm*(PI2/4);for(let j=0;j<6;j++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(armA)*bs*(0.4+j*0.15),vy:Math.sin(armA)*bs*(0.4+j*0.15),size:4,dmg:dmg*0.35});}}}break;}
      default:break;
    }
  ...
  function update(dt){
    ...
    for(let i=gs.pBullets.length-1;i>=0;i--){const b=gs.pBullets[i];for(let j=gs.enemies.length-1;j>=0;j--){const e=gs.enemies[j];if(dist(b,e)<b.size+e.size){
      ...
      if(b._bounced&&hasAU(gs,"ricochet_mastery")){const _sa=Math.atan2(b.vy,b.vx);const _sdPri=p.damage*2.0;const _sdSec=p.damage*1.2;gs._slices=(gs._slices||[]);gs._slices.push({x:e.x,y:e.y,a:_sa,life:18,ml:18,len:120});for(let _si=0;_si<8;_si++){const _soff=rand(-0.3,0.3);gs.particles.push({x:e.x+Math.cos(_sa+_soff)*rand(10,60),y:e.y+Math.sin(_sa+_soff)*rand(10,60),vx:Math.cos(_sa+_soff)*rand(1,3),vy:Math.sin(_sa+_soff)*rand(1,3),life:rand(12,25),ml:25,color:_si%2===0?"#ff4466":"#ffaa88",size:rand(2,4)});}
        e.hp-=_sdPri;trackDmg(gs,"Rage Slice",_sdPri);if(metaRef.current.showHitText!==false)gs.hitTexts.push({x:e.x+rand(-6,6),y:e.y-e.size,text:Math.round(_sdPri),life:24,ml:24,col:"#ff4466"});sp(gs,e.x,e.y,"#ff4466",4,3);
        let _rsFirst=true;gs.enemies.forEach((se,si)=>{if(se!==e){const _sx=e.x+Math.cos(_sa)*60,_sy=e.y+Math.sin(_sa)*60;if(dist(se,{x:_sx,y:_sy})<60+se.size){const _rsd=_rsFirst?_sdPri:_sdSec;_rsFirst=false;se.hp-=_rsd;trackDmg(gs,"Rage Slice",_rsd);if(metaRef.current.showHitText!==false)gs.hitTexts.push({x:se.x+rand(-6,6),y:se.y-se.size,text:Math.round(_rsd),life:24,ml:24,col:"#ff4466"});sp(gs,se.x,se.y,"#ff4466",8,4);if(se.hp<=0)killE(gs,se,si);}}});}
    ...
    if(p.hp<=0&&!gs.isTutorial){p.alive=false;sp(gs,p.x,p.y,"#00e5ff",22,5);gs.screenShake=16;
      ...
      else{const _rawEe=Math.max(0,Math.floor(gs.wave*1.5+gs.kills*0.38+Math.pow(gs.wave,2.8)*0.065+Math.pow(gs.wave,1.8)*0.4)-(gs.kills===0&&gs.wave<=1?1:0));const _heTier=metaRef.current.metaTier||1;const _heTV=_heTier===3?2.5:_heTier===2?1.5:1;const _hePh=_heTier>=2?(1+(metaRef.current.phantomHighWave||0)*(0.01+(metaRef.current.lab?.completed?.phantom_enhance||0)*0.001)):1;const _hePr=_heTier>=2?(1+(metaRef.current.practiseHighWave||0)*(0.006+(metaRef.current.lab?.completed?.practise_enhance||0)*0.001)):1;const _heEnf=_heTier>=2?1+(metaRef.current.enforcerKills||0)*0.025:1;const _heDes=_heTier>=2?1+((metaRef.current.ownedDesigns||[]).length)*0.004:1;const _heMult=_heTV*_hePh*_hePr*_heEnf*_heDes;const ee=Math.floor(_rawEe*_heMult*(gs._diffuseMult||1));setDeathData({wave:gs.wave,kills:gs.kills,echoesEarned:ee,cause:gs.deathCause||"Unknown"});
  ...
  function render(){
    ...
    if(p.alive){const blink=p.invTimer>0&&Math.floor(p.invTimer/40)%2===0;const sc=gs.shipCol||{color:"#00e5ff",glow:"#00e5ff"};if(!blink){
      ...
      ctx.beginPath();ctx.moveTo(p.x,p.y-p.size-4);ctx.lineTo(p.x-p.size,p.y+p.size);ctx.lineTo(p.x,p.y+p.size*0.4);ctx.lineTo(p.x+p.size,p.y+p.size);ctx.closePath();ctx.fill();ctx.shadowBlur=0;/* custom design overlay */const _sd=SHIP_DESIGNS.find(d=>d.id===(metaRef.current.shipDesign||"none"));if(_sd&&_sd.draw){const _dcId=metaRef.current.designColor||metaRef.current.shipColor||"cyan";const _dcObj=SHIP_COLORS.find(c=>c.id===_dcId)||SHIP_COLORS[0];_sd.draw(ctx,p.x,p.y,p.size,_dcObj.color);ctx.globalAlpha=1;}
  ...
  const ShipDisplay=({onClick,size=60})=>{
    ...
    useEffect(()=>{const c=ref.current;if(!c)return;const ctx=c.getContext("2d");ctx.clearRect(0,0,size,size);
      ...
      ctx.shadowBlur=0;/* design overlay */const _sdM=SHIP_DESIGNS.find(d=>d.id===(metaRef.current.shipDesign||"none"));if(_sdM&&_sdM.draw){const _dcIdM=metaRef.current.designColor||metaRef.current.shipColor||"cyan";const _dcM=SHIP_COLORS.find(c2=>c2.id===_dcIdM)||SHIP_COLORS[0];_sdM.draw(ctx,cx,cy,s,_dcM.color);ctx.globalAlpha=1;ctx.shadowBlur=0;}ctx.fillStyle=sc.color;ctx.globalAlpha=0.3;ctx.beginPath();ctx.arc(cx,cy+s+3,3,0,PI2);ctx.fill();ctx.globalAlpha=1;
  ...
  const[codexOpen,setCodexOpen]=useState({controls:false,mechanics:false,currencies:false,enemies:false,abilities:false,scrap:false,cores:false,plasma:false,meta:false,metaab:false,designs:false,labs:false});
...
<CodexSec id="labs" title={"META LAB UPGRADES"} color="#44ccaa">
  ...
  return(
    <div style={{width:"100%",height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",fontFamily:"'Courier New', monospace",position:"relative",background:"#06060e"}}>
      ...
      `}</style>      <div style={{position:"relative",width:GW*sc2,height:GH*sc2,outline:meta.showBorder!==false?"1px solid #2a2a3a":"none",border:"none",zIndex:1}}>
        ...
        {paused&&phase==="playing"&&!showWiki&&(
          <div className="vs-scroll vs-fade" style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(6,6,14,0.88)",zIndex:15,overflow:"auto",padding:"20px 0"}}>
            ...
            {(()=>{const _mr=metaRef.current;const _t=_mr.metaTier||1;if(_t<2)return null;const _phW=_mr.phantomHighWave||0;const _prW=_mr.practiseHighWave||0;const _phL=(_mr.lab?.completed?.phantom_enhance||0)*0.001;const _prL=(_mr.lab?.completed?.practise_enhance||0)*0.001;const _tv=_t===3?2.5:_t===2?1.5:1;const _enf2=_t>=2?1+(_mr.enforcerKills||0)*0.025:1;const _des3=_t>=2?1+((_mr.ownedDesigns||[]).length)*0.004:1;const _em=_tv*(_t>=2?(1+_phW*(0.01+_phL)):1)*(_t>=2?(1+_prW*(0.006+_prL)):1)*_enf2*_des3*(gsRef.current?._diffuseMult||1);return(
            ...
            <div style={{display:"flex",gap:10,marginTop:16,flexDirection:"column",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:8,marginTop:2}}>
                <button onClick={()=>{setShowWiki(true);setCodexOpen({controls:false,mechanics:false,currencies:false,enemies:false,abilities:false,scrap:false,cores:false,plasma:false,meta:false,metaab:false,designs:false,labs:false})}} style={{...bs2("#55667744"),padding:"8px 16px",fontSize:11,borderWidth:1,color:"#8899aa"}}>CODEX</button>
                ...
                <button onClick={()=>{_returnToPauseRef.current=true;pausedRef.current=false;setPhase("settings");}} style={{...bs2("#55667744"),padding:"8px 16px",fontSize:11,borderWidth:1,color:"#8899aa"}}>SETTINGS</button>
              </div>
            </div>
            ...
            {showPauseLabs&&(
              <div onClick={()=>setShowPauseLabs(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",zIndex:30,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
                <div onClick={e=>e.stopPropagation()} style={{background:"#0a0a18",border:"1px solid #ff884433",borderRadius:6,padding:"16px 14px",maxWidth:380,width:"100%",maxHeight:"80vh",overflow:"auto"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    <div style={{color:"#ff9966",fontSize:15,fontWeight:"bold",letterSpacing:2}}>LABS</div>
                    <button onClick={()=>setShowPauseLabs(false)} style={{background:"none",border:"1px solid #334455",color:"#8899aa",padding:"2px 8px",cursor:"pointer",fontFamily:"inherit",fontSize:10,borderRadius:3}}>✕</button>
                  </div>
        ...
        {phase==="menu"&&!showWiki&&(
          <div className="vs-scroll" style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#06060e",zIndex:10,overflow:"auto",padding:"20px 0"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:6,justifyContent:"center",zIndex:1,position:"relative"}}>
              <button onClick={()=>{setShowWiki(true);setCodexOpen({controls:false,mechanics:false,currencies:false,enemies:false,abilities:false,scrap:false,cores:false,plasma:false,meta:false,metaab:false,designs:false,labs:false})}} style={{...bs2("#44aacc33"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#66bbcc"}}>CODEX</button>
              ...
              {<button onClick={()=>{setHeInfoId(null);setAbInfoId(null);setSprintInfo(false);setPhase("hyperecho");}} style={{...bs2("#ffcc4433"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#ffcc44"}}>HYPERECHO</button>}
              ...
              <div style={{width:"100%",maxWidth:GW-20,marginTop:14}}>
                ...
                <div style={{display:"flex"}}>
                  ...
                  <div style={{flex:1,position:"relative"}}>
                    ...
                    {/* Trendline overlay */}
                    {_filtered.length>=3&&(()=>{/* Calculate hyperecho multiplier for each run by reverse-engineering from raw echoes */const _heData=_filtered.map(r=>{const w=r.wave||0,k=r.kills||0;const rawE=Math.max(1,Math.floor(w*1.5+k*0.38+Math.pow(w,2.8)*0.065+Math.pow(w,1.8)*0.4)-(k===0&&w<=1?1:0));const actualE=r.echoes||0;return actualE>0&&rawE>0?actualE/rawE:1;});const _heMax=Math.max(0.001,..._heData);const _showHe=(meta.metaTier||1)>=2&&historyHideForfeits;return <svg style={{position:"absolute",top:0,left:0,width:"100%",height:_chartH,pointerEvents:"none"}} viewBox={`0 0 1000 ${_chartH}`} preserveAspectRatio="none">
        ...
        {phase==="hyperecho"&&(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"#06060e",zIndex:10,overflow:"hidden"}}>
            ...
            <div style={{marginTop:20,width:"100%",maxWidth:360,display:"flex",flexDirection:"column",gap:6,position:"relative"}}>
              {(()=>{const _tier=meta.metaTier||1;const _phW=meta.phantomHighWave||0;const _prW=meta.practiseHighWave||0;const _phLab=(meta.lab?.completed?.phantom_enhance||0)*0.001;const _prLab=(meta.lab?.completed?.practise_enhance||0)*0.001;const _phV=1+_phW*(0.01+_phLab);const _prV=1+_prW*(0.006+_prLab);const _mults=[{id:"tier",name:"Tier",desc:"",color:"#bb99ff",val:_tier===3?2.5:_tier===2?1.5:1},{id:"phantom",name:"Phantom",desc:`+${(0.01+_phLab).toFixed(3)} per phantom max wave (${_phW}).`,color:"#cc66cc",val:_tier>=2?_phV:1},{id:"practise",name:"Practise",desc:`+${(0.006+_prLab).toFixed(3)} per practise max wave (${_prW}).`,color:"#cc9966",val:_tier>=2?_prV:1},{id:"enforcer",name:"Enforcer",desc:"+0.025 per enforcer defeated in Playground.",color:"#ff5577",val:_tier>=2?1+(meta.enforcerKills||0)*0.025:1},{id:"designer",name:"Designer",desc:"+0.004 per custom ship design owned. Unlock designs with boss shards in Settings > Custom Designs.",color:"#cc7744",val:_tier>=2?1+((meta.ownedDesigns||[]).length)*0.004:1},{id:"diffusion",name:"Diffusion",desc:"Per-run echo bonus from diffusing ability slots. Each diffuse adds +" + ((metaRef.current.lab?.completed?.diffusion_multi||0)*0.009+0.0375).toFixed(3) + " to the multiplier. Resets each run.",color:"#44ddcc",val:gsRef.current?._diffuseMult||1}];const _total=_mults.reduce((a,m)=>a*m.val,1);return _mults.map(m=>(
        ...
        {phase==="settings"&&(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"#06060e",zIndex:10,overflow:"hidden"}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center",justifyContent:"center",maxWidth:300}}>
              {!syncCode?<>
                ...
                {!confirmDisconnect?<button onClick={()=>setConfirmDisconnect(true)} style={{padding:"6px 14px",background:"#141428",border:"1px solid #ff334466",borderRadius:4,cursor:"pointer",fontFamily:"inherit",fontSize:10,color:"#cc6666",letterSpacing:1}}>DISCONNECT</button>:<div style={{display:"flex",gap:4,alignItems:"center"}}><button onClick={()=>{const _ms=metaRef.current;const _ts=Date.now();const _pushFinal={..._ms,savedAt:_ts};try{const _h=localStorage.getItem("vs4-history")||"[]";const _t=localStorage.getItem("vs4-tut")||"0";fetch(SUPABASE_URL+"/rest/v1/saves",{method:"POST",headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify({code:syncCodeRef.current,data:{meta:_pushFinal,history:JSON.parse(_h),tut:_t,session_id:"disconnected"},updated_at:new Date().toISOString()})}).catch(()=>{});}catch(_e){}setSyncCode(null);syncCodeRef.current=null;setSyncStatus("none");setSyncCodeInput("");setConfirmDisconnect(false);setSessionTakeover(false);try{localStorage.removeItem("vs4-sync-code");localStorage.removeItem("vs4-meta");localStorage.removeItem("vs4-history");localStorage.removeItem("vs4-tut");}catch(e){}const _def={echoes:0,levels:{},shards:0,shardsBought:0,abUpgrades:{},shipColor:"cyan",bulletColor:"teal",bossShards:0,ownedDesigns:[],shipDesign:"none",designColor:undefined,showNewEnemy:true,showHitText:true,showMagnetRange:true,highWave:0};setMeta(_def);setShowTutPrompt(true);}} style={{padding:"4px 10px",background:"#1a0a0a",border:"1px solid #ff3344",borderRadius:3,cursor:"pointer",fontFamily:"inherit",fontSize:9,color:"#ff4455"}}>CONFIRM</button><button onClick={()=>setConfirmDisconnect(false)} style={{padding:"4px 10px",background:"none",border:"1px solid #33445544",borderRadius:3,cursor:"pointer",fontFamily:"inherit",fontSize:9,color:"#667788"}}>CANCEL</button></div>}
                <div onClick={()=>setShowSyncInfo(true)} style={{width:15,height:15,borderRadius:"50%",border:"1px solid #44ccaa44",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:8,color:"#44ccaa66",flexShrink:0}} onMouseOver={e=>e.currentTarget.style.color="#44ccaa"} onMouseOut={e=>e.currentTarget.style.color="#44ccaa66"}>?</div>
              </>}
            </div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",maxWidth:380}}>
              ...
              <button onClick={()=>setShowDesignPopup(true)} style={{padding:"10px 14px",flex:1,minWidth:90,background:"#0a0a16",border:"1px solid #44ccaa44",borderRadius:4,cursor:"pointer",fontFamily:"inherit",fontSize:9,color:"#88ccaa",textAlign:"left",transition:"all 0.2s",display:"flex",alignItems:"center",justifyContent:"space-between"}} onMouseOver={e=>e.currentTarget.style.borderColor="#44ccaa"} onMouseOut={e=>e.currentTarget.style.borderColor="#44ccaa44"}><span>Custom Designs</span><span style={{color:"#44ccaa44",fontSize:11}}>›</span></button>
            </div>
            {showDesignPopup&&(()=>{const _dCol=SHIP_COLORS.find(c=>c.id===(meta.designColor||meta.shipColor||"cyan"))||SHIP_COLORS[0];return(
            <div onClick={()=>setShowDesignPopup(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",zIndex:30,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
              <div onClick={e=>e.stopPropagation()} className="vs-scroll" style={{background:"#0a0a18",border:"1px solid #cc333333",borderRadius:6,padding:"16px 14px",maxWidth:420,width:"100%",maxHeight:"80vh",overflow:"auto"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <div style={{color:"#cc3333",fontSize:17,fontWeight:"bold",letterSpacing:2}}>CUSTOM DESIGNS</div>
                  <button onClick={()=>setShowDesignPopup(false)} style={{background:"none",border:"1px solid #334455",color:"#8899aa",padding:"4px 10px",cursor:"pointer",fontFamily:"inherit",fontSize:11,borderRadius:3}}>✕</button>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:4}}>
                  {SHIP_COLORS.map(sc=>{const _isShipCol=(meta.shipColor||"cyan")===sc.id;const sel=(meta.designColor||(_isShipCol?"":"cyan"))===sc.id&&!_isShipCol;return(
          ...
          <h3 style={{color:"#ddeeff",fontSize:11,letterSpacing:2,margin:"20px 0 8px"}}>MOBILE CONTROLS</h3>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4,marginTop:6}}>
              <span style={{color:"#334455",fontSize:8,letterSpacing:1}}>Version 2.2.1</span>
            ...
            {showChangelog&&(
              <div onClick={()=>setShowChangelog(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",zIndex:30,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
                <div onClick={e=>e.stopPropagation()} className="vs-scroll" style={{background:"#0a0a18",border:"1px solid #44ccaa33",borderRadius:6,padding:"16px 14px",maxWidth:380,width:"100%",maxHeight:"80vh",overflow:"auto"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <div style={{color:"#ccddee",fontSize:14,fontWeight:"bold",letterSpacing:2}}>CHANGELOG</div>
                    <button onClick={()=>setShowChangelog(false)} style={{background:"none",border:"1px solid #334455",color:"#8899aa",padding:"2px 8px",cursor:"pointer",fontFamily:"inherit",fontSize:10,borderRadius:3}}>✕</button>
                  </div>
                  <div style={{color:"#44ccaa",fontSize:13,fontWeight:"bold",letterSpacing:1,marginBottom:8}}>v2.2.1</div>
                  <div style={{color:"#99aabb",fontSize:10,lineHeight:1.8,marginBottom:12}}>
                    • New lab: Sprint Currency Lifespan<br/>
                    • Diffusion now properly removes ability slots<br/>
                    • Enforcer rebalancing (wraith harder, fortress easier)<br/>
                    • Sprint improvements and bug fixes<br/>
                    • Lab cost rebalancing and UI tweaks<br/>
                  </div>
                  <button onClick={()=>setClExpanded(p=>({...p,v22:!p.v22}))} style={{width:"100%",background:"none",border:"1px solid #334455",color:"#778899",padding:"6px 10px",cursor:"pointer",fontFamily:"inherit",fontSize:10,borderRadius:3,textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div className="vs-fade" style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"rgba(6,6,14,0.93)",zIndex:10,overflow:"hidden"}}>
            <div style={{padding:"10px 14px",borderBottom:"1px solid #1a1a2e",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:6}}>
                ...
                <button onClick={()=>{setShowWiki(true);setCodexOpen({controls:false,mechanics:false,currencies:false,enemies:false,abilities:false,scrap:false,cores:false,plasma:false,meta:false,metaab:false,designs:false,labs:false})}} style={{background:"none",border:"1px solid #334455",color:"#778899",padding:"3px 8px",cursor:"pointer",fontFamily:"inherit",fontSize:9}}>CODEX</button>
              </div>
            </div>
      ...
      {sessionTakeover&&syncCode&&<div style={{position:"absolute",inset:0,background:"rgba(2,2,8,0.92)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
        <div style={{background:"#0a0a18",border:"1px solid #ff884444",borderRadius:8,padding:"24px 20px",maxWidth:340,width:"100%",textAlign:"center"}}>
          ...
          <div style={{color:"#99aabb",fontSize:10,lineHeight:1.7,marginBottom:16}}>
            Another device has connected to save <span style={{color:"#44ccaa",fontFamily:"monospace",letterSpacing:2}}>{syncCode}</span>. Only one device can play at a time.
          </div>
          <div style={{display:"flex",gap:8,justifyContent:"center"}}>
            <button onClick={()=>{const code=syncCodeRef.current;if(!code||!_SYNC_OK)return;fetch(SUPABASE_URL+"/rest/v1/saves?code=eq."+code+"&select=data",{headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY}}).then(r=>r.json()).then(rows=>{if(rows.length>0&&rows[0].data&&rows[0].data.meta){const cd=rows[0].data;setMeta(cd.meta);try{localStorage.setItem("vs4-meta",JSON.stringify(cd.meta));}catch(e){}if(cd.history)try{localStorage.setItem("vs4-history",JSON.stringify(cd.history));}catch(e){}if(cd.tut)try{localStorage.setItem("vs4-tut",cd.tut);}catch(e){}const _tsTC=Date.now();const _msTC={...cd.meta,savedAt:_tsTC};try{const _hTC=JSON.stringify(cd.history||[]);const _tTC=cd.tut||"0";fetch(SUPABASE_URL+"/rest/v1/saves",{method:"POST",headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify({code,data:{meta:_msTC,history:cd.history||[],tut:_tTC,session_id:_sessionIdRef.current},updated_at:new Date().toISOString()})}).catch(()=>{});}catch(_e){}}setSessionTakeover(false);setPhase("menu");}).catch(()=>{setSessionTakeover(false);});}} style={{padding:"8px 18px",background:"transparent",border:"1px solid #44ccaa66",borderRadius:4,cursor:"pointer",fontFamily:"inherit",fontSize:10,color:"#88ccaa",letterSpacing:1,transition:"all 0.2s"}} onMouseOver={e=>{e.currentTarget.style.background="#44ccaa";e.currentTarget.style.color="#06060e";}} onMouseOut={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="#88ccaa";}}>TAKE CONTROL</button>
            <button onClick={()=>{setSyncCode(null);syncCodeRef.current=null;setSyncStatus("none");setSyncCodeInput("");setConfirmDisconnect(false);setSessionTakeover(false);try{localStorage.removeItem("vs4-sync-code");localStorage.removeItem("vs4-meta");localStorage.removeItem("vs4-history");localStorage.removeItem("vs4-tut");}catch(e){}const _def2={echoes:0,levels:{},shards:0,shardsBought:0,abUpgrades:{},shipColor:"cyan",bulletColor:"teal",bossShards:0,ownedDesigns:[],shipDesign:"none",designColor:undefined,showNewEnemy:true,showHitText:true,showMagnetRange:true,highWave:0};setMeta(_def2);setShowTutPrompt(true);setPhase("menu");}} style={{padding:"8px 18px",background:"transparent",border:"1px solid #ff334466",borderRadius:4,cursor:"pointer",fontFamily:"inherit",fontSize:10,color:"#cc6666",letterSpacing:1,transition:"all 0.2s"}} onMouseOver={e=>{e.currentTarget.style.background="#ff3344";e.currentTarget.style.color="#06060e";}} onMouseOut={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="#cc6666";}}>LOG OUT</button>
          </div>
        </div>
      </div>}
      </div>
