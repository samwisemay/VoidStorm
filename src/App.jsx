...
export default function VoidStorm(){
  ...
  const[syncCode,setSyncCode]=useState(null);const[syncStatus,setSyncStatus]=useState("none");const[syncCodeInput,setSyncCodeInput]=useState("");const[showShipPopup,setShowShipPopup]=useState(false);const[showBulletPopup,setShowBulletPopup]=useState(false);const[showBgPopup,setShowBgPopup]=useState(false);const[showSyncInfo,setShowSyncInfo]=useState(false);const[syncConflict,setSyncConflict]=useState(null);const[showDesignPopup,setShowDesignPopup]=useState(false);const[showChangelog,setShowChangelog]=useState(false);const syncCodeRef=useRef(null);const _cloudDebounce=useRef(null);
  ...
  function offerAb(gs){
    ...
    const _dcLvl3=metaRef.current.lab?.completed?.diffusion_chance||0;const _dcPct3=_dcLvl3>0?_dcLvl3*10:0;const _dmLvl3=metaRef.current.lab?.completed?.diffusion_multi||0;const _dmBonus3=0.05+_dmLvl3*0.012;gs._showDiffuse=!gs.isTutorial&&!gs.isNewMode&&!gs.isPlayground&&(metaRef.current.metaTier||1)>=2&&_dcPct3>0&&Math.random()*100<_dcPct3;gs._diffuseBonus=_dmBonus3;setAbChoices(picks);setPhase("ability");
  }

  function pickAb(id){
    const gs=gsRef.current;if(!gs)return;
    gs._sprintJustEnded=false;gs._sprintBannerShown=false;
  ...
  function fireEB(gs,e){
    ...
    switch(e.pattern){
      ...
      case"enf_wraith":{/* Phase assault - teleport bursts + multi-spread. Fires fast spreads from multiple angles */const _wp=Math.floor(gs.time/350)%3;const _wOffsets=[0,PI2/3,-PI2/3];const _wBase=_wOffsets[_wp];for(let i=-2;i<=2;i++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+_wBase+i*0.15)*bs*1.2,vy:Math.sin(a+_wBase+i*0.15)*bs*1.2,size:5,dmg:dmg*0.6});}/* plus orbiting shots */const _wo=gs.time*0.005;for(let i=0;i<3;i++){const _oa=_wo+i*(PI2/3);gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(_oa)*bs*0.7,vy:Math.sin(_oa)*bs*0.7,size:4,dmg:dmg*0.4});}/* fast aimed */gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*bs*2.0,vy:Math.sin(a)*bs*2.0,size:3,dmg:dmg*0.9});break;}
      case"enf_fortress":{/* Siege mode - rotating shield wall of slow bullets + aimed artillery */const _fRot=gs.time*0.002;/* outer wall ring rotating slowly */const _fN=18;for(let i=0;i<_fN;i++){const ra=(PI2/_fN)*i+_fRot;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra)*bs*0.35,vy:Math.sin(ra)*bs*0.35,size:6,dmg:dmg*0.3});}/* inner fast ring, opposite rotation */const _fN2=10;for(let i=0;i<_fN2;i++){const ra2=(PI2/_fN2)*i-_fRot*1.5;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra2)*bs*0.8,vy:Math.sin(ra2)*bs*0.8,size:4,dmg:dmg*0.5});}/* heavy aimed artillery */for(let i=-1;i<=1;i++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+i*0.15)*bs*1.5,vy:Math.sin(a+i*0.15)*bs*1.5,size:7,dmg:dmg*0.8});}break;}
  ...
  function killE(gs,e,idx){/* Boss shard drop */if(e.type==="boss"&&!gs._isEnforcer){const _bsLvl=metaRef.current.lab?.completed?.boss_shard_drop||0;const _bsPct=_bsLvl*8;if(_bsPct>0&&Math.random()*100<_bsPct){gs.pickups.push({x:e.x,y:e.y,type:"bossShards",value:1,life:600,vx:rand(-1,1),vy:rand(-3,-1),size:12,color:"#cc3333",icon:"◈"});}}
    ...
    if(!gs.waveActive&&gs._waveEndTimer>0){gs._waveEndTimer-=dt;if(gs._waveEndTimer<=0){gs._waveEndTimer=0;if(has(gs,"drone")&&hasAU(gs,"drone_mastery")){gs.pickups.forEach(pk=>{if(!pk._counted){pk._counted=true;if(!gs._missed)gs._missed={};gs._missed[pk.type]=(gs._missed[pk.type]||0)+pk.value;}});}if(gs._inSprint&&gs.pickups.length>0){gs.pickups.forEach(pk=>{const _sc=CUR[pk.type]?.color||"#fff";for(let _si=0;_si<10;_si++){const _sa=PI2/10*_si+rand(-0.4,0.4);const _sv=rand(3,7.5);gs.particles.push({x:pk.x,y:pk.y,vx:Math.cos(_sa)*_sv,vy:Math.sin(_sa)*_sv,life:rand(16,28),ml:28,color:_sc,size:rand(5,9)});}for(let _si=0;_si<5;_si++){gs.particles.push({x:pk.x+rand(-12,12),y:pk.y+rand(-12,12),vx:rand(-1.5,1.5),vy:rand(-4,-1.5),life:rand(20,35),ml:35,color:"#ffffff",size:rand(3,5.5)});}});}gs.pickups=[];
      if(gs.isPlayground){
        ...
        if(_isPct2>0&&gs.wave<_isThresh2&&!gs.isTutorial&&!gs.isPlayground&&!gs.isNewMode&&!gs.isPractise&&!metaRef.current.introSprintOff){
          ...
          {const _seLvl=metaRef.current.lab?.completed?.sprint_efficiency||0;const _sePct=_seLvl>0?[40,50,60,70,80][Math.min(_seLvl-1,4)]:30;if(Math.random()*100<_sePct){gs._sprintLabWaves=(gs._sprintLabWaves||0)+1;setMeta(prev=>{const lab=prev.lab;if(!lab||!lab.active||lab.active.length===0)return prev;const nx={...prev,lab:{...lab,totalWaves:(lab.totalWaves||0)+1,active:[...lab.active],completed:{...(lab.completed||{})}}};nx.lab.active=nx.lab.active.map(ar=>{if(!ar)return null;const upd={...ar,wavesProgress:(ar.wavesProgress||0)+1};if(upd.wavesProgress>=upd.wavesNeeded){const lu=LAB_UPGRADES.find(l=>l.id===ar.id);const curLvl=(nx.lab.completed[ar.id]||0);nx.lab.completed[ar.id]=curLvl+1;if(lu&&curLvl+1<lu.levels.length)return{id:ar.id,wavesNeeded:lu.levels[curLvl+1].waves,wavesProgress:0};return null;}return upd;}).filter(Boolean);saveMeta(nx);return nx;});}}
      ...
      `}</style>      <div style={{position:"relative",width:GW*sc2,height:GH*sc2,outline:meta.showBorder!==false?"1px solid #2a2a3a":"none",border:"none",zIndex:1}}>
        ...
        {phase==="ability"&&(
          <div className="vs-fade" style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(6,6,14,0.92)",zIndex:10}}>
            {gsRef.current?._sprintJustEnded&&!gsRef.current?._sprintBannerShown&&(()=>{gsRef.current._sprintBannerShown=true;return <div style={{color:"#44ccaa",fontSize:22,fontWeight:"bold",letterSpacing:5,marginBottom:8,textShadow:"0 0 12px #44ccaa44"}}>INTRO SPRINT OVER</div>;})()}
        ...
        {phase==="hyperecho"&&(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"#06060e",zIndex:10,overflow:"hidden"}}>
            ...
            <div style={{marginTop:20,width:"100%",maxWidth:360,display:"flex",flexDirection:"column",gap:6,position:"relative"}}>
              {(()=>{const _tier=meta.metaTier||1;const _phW=meta.phantomHighWave||0;const _prW=meta.practiseHighWave||0;const _phLab=(meta.lab?.completed?.phantom_enhance||0)*0.001;const _prLab=(meta.lab?.completed?.practise_enhance||0)*0.001;const _phV=1+_phW*(0.01+_phLab);const _prV=1+_prW*(0.006+_prLab);const _mults=[{id:"tier",name:"Tier",desc:"",color:"#bb99ff",val:_tier===3?2.5:_tier===2?1.5:1},{id:"phantom",name:"Phantom",desc:`+${(0.01+_phLab).toFixed(3)} per phantom max wave (${_phW}).`,color:"#cc66cc",val:_tier>=2?_phV:1},{id:"practise",name:"Practise",desc:`+${(0.006+_prLab).toFixed(3)} per practise max wave (${_prW}).`,color:"#cc9966",val:_tier>=2?_prV:1},{id:"enforcer",name:"Enforcer",desc:"+0.025 per enforcer defeated in Playground.",color:"#ff5577",val:_tier>=2?1+(meta.enforcerKills||0)*0.025:1},{id:"designer",name:"Designer",desc:"+0.004 per custom ship design owned. Unlock designs with boss shards in Settings > Custom Designs.",color:"#cc7744",val:_tier>=2?1+((meta.ownedDesigns||[]).length)*0.004:1},{id:"diffusion",name:"Diffusion",desc:"Per-run echo bonus from diffusing ability slots. Each diffuse adds +" + ((metaRef.current.lab?.completed?.diffusion_multi||0)*0.012+0.05).toFixed(3) + " to the multiplier. Resets each run.",color:"#44ddcc",val:gsRef.current?._diffuseMult||1}];const _total=_mults.reduce((a,m)=>a*m.val,1);return _mults.map(m=>(
        ...
        {phase==="settings"&&(
          ...
          <h3 style={{color:"#ddeeff",fontSize:11,letterSpacing:2,margin:"20px 0 8px"}}>MOBILE CONTROLS</h3>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4,marginTop:6}}>
              <span style={{color:"#334455",fontSize:8,letterSpacing:1}}>Version 2.2</span>
            ...
            {showChangelog&&(
              <div onClick={()=>setShowChangelog(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",zIndex:30,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
                <div onClick={e=>e.stopPropagation()} className="vs-scroll" style={{background:"#0a0a18",border:"1px solid #44ccaa33",borderRadius:6,padding:"16px 14px",maxWidth:380,width:"100%",maxHeight:"80vh",overflow:"auto"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <div style={{color:"#ccddee",fontSize:14,fontWeight:"bold",letterSpacing:2}}>VERSION 2.2</div>
                  ...
                  <div style={{color:"#44ccaa",fontSize:11,fontWeight:"bold",letterSpacing:1,marginBottom:6}}>NEW</div>
                  <div style={{color:"#99aabb",fontSize:10,lineHeight:1.8}}>
                  ...
                  <div style={{color:"#cc8844",fontSize:11,fontWeight:"bold",letterSpacing:1,marginTop:10,marginBottom:6}}>FIXES</div>
                  <div style={{color:"#99aabb",fontSize:10,lineHeight:1.8}}>
