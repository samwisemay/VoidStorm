...
export default function VoidStorm(){
  ...
  function update(dt){
    ...
    if(p.hp<=0&&!gs.isTutorial){p.alive=false;sp(gs,p.x,p.y,"#00e5ff",22,5);gs.screenShake=16;
      ...
      else{const _rawEe=Math.max(0,Math.floor(gs.wave*1.5+gs.kills*0.38+Math.pow(gs.wave,2.8)*0.065+Math.pow(gs.wave,1.8)*0.4)-(gs.kills===0&&gs.wave<=1?1:0));const _heTier=metaRef.current.metaTier||1;const _heTV=_heTier===3?2.5:_heTier===2?1.5:1;const _hePh=_heTier>=2?(1+(metaRef.current.phantomHighWave||0)*(0.01+(metaRef.current.lab?.completed?.phantom_enhance||0)*0.001)):1;const _hePr=_heTier>=2?(1+(metaRef.current.practiseHighWave||0)*(0.006+(metaRef.current.lab?.completed?.practise_enhance||0)*0.001)):1;const _heEnf=_heTier>=2?1+(metaRef.current.enforcerKills||0)*0.025:1;const _heMult=_heTV*_hePh*_hePr*_heEnf;const ee=Math.floor(_rawEe*_heMult*(gs._diffuseMult||1));setDeathData({wave:gs.wave,kills:gs.kills,echoesEarned:ee,cause:gs.deathCause||"Unknown"});
      ...
      `}</style>      <div style={{position:"relative",width:GW*sc2,height:GH*sc2,border:meta.showBorder!==false?"1px solid #2a2a3a":"none",zIndex:1}}>
        ...
        {paused&&phase==="playing"&&!showWiki&&(
          <div className="vs-scroll vs-fade" style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(6,6,14,0.88)",zIndex:15,overflow:"auto",padding:"20px 0"}}>
            ...
            {(()=>{const _mr=metaRef.current;const _t=_mr.metaTier||1;if(_t<2)return null;const _phW=_mr.phantomHighWave||0;const _prW=_mr.practiseHighWave||0;const _phL=(_mr.lab?.completed?.phantom_enhance||0)*0.001;const _prL=(_mr.lab?.completed?.practise_enhance||0)*0.001;const _tv=_t===3?2.5:_t===2?1.5:1;const _enf2=_t>=2?1+(_mr.enforcerKills||0)*0.025:1;const _em=_tv*(_t>=2?(1+_phW*(0.01+_phL)):1)*(_t>=2?(1+_prW*(0.006+_prL)):1)*_enf2;return(
            ...
            {heInfoId==="pause_echo"&&(()=>{const _mr2=metaRef.current;const _t2=_mr2.metaTier||1;const _phW2=_mr2.phantomHighWave||0;const _prW2=_mr2.practiseHighWave||0;const _phL2=(_mr2.lab?.completed?.phantom_enhance||0)*0.001;const _prL2=(_mr2.lab?.completed?.practise_enhance||0)*0.001;const _phV2=1+_phW2*(0.01+_phL2);const _prV2=1+_prW2*(0.006+_prL2);const _pMults=[{name:"Tier Bonus",color:"#bb99ff",val:_t2===3?2.5:_t2===2?1.5:1},{name:"Phantom",color:"#cc66cc",val:_t2>=2?_phV2:1},{name:"Practise",color:"#cc9966",val:_t2>=2?_prV2:1},{name:"Enforcer",color:"#ff5577",val:_t2>=2?1+(_mr2.enforcerKills||0)*0.025:1},{name:"Diffusion",color:"#44ddcc",val:1}];const _pTot=_pMults.reduce((a,m)=>a*m.val,1);return(
        ...
        {phase==="hyperecho"&&(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"#06060e",zIndex:10,overflow:"hidden"}}>
            ...
            <div style={{width:"100%",maxWidth:360,borderTop:"1px solid #44556644",marginTop:14,paddingTop:12,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 12px 0"}}>
              ...
              {(()=>{const _tier=meta.metaTier||1;const _phW2=meta.phantomHighWave||0;const _prW2=meta.practiseHighWave||0;const _phL2=(meta.lab?.completed?.phantom_enhance||0)*0.001;const _prL2=(meta.lab?.completed?.practise_enhance||0)*0.001;const _tv=_tier===3?2.5:_tier===2?1.5:1;const _enfK=meta.enforcerKills||0;const _total=_tv*(_tier>=2?(1+_phW2*(0.01+_phL2)):1)*(_tier>=2?(1+_prW2*(0.006+_prL2)):1)*(_tier>=2?1+_enfK*0.025:1)*1;return <div style={{color:"#ffcc44",fontSize:26,fontWeight:"bold",fontFamily:"monospace"}}>×{_total.toFixed(3)}</div>;})()}
            </div>
        ...
        {phase==="settings"&&(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"#06060e",zIndex:10,overflow:"hidden"}}>
            ...
            <div style={{color:"#334455",fontSize:7,marginTop:6,letterSpacing:1}}>Version 2.1.6</div>
          {showShipPopup&&(
