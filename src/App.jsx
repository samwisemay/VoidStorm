...
export default function VoidStorm(){
 ...
 const[syncCode,setSyncCode]=useState(null);const[syncStatus,setSyncStatus]=useState("none");const[syncCodeInput,setSyncCodeInput]=useState("");const[showShipPopup,setShowShipPopup]=useState(false);const[showBulletPopup,setShowBulletPopup]=useState(false);const[showBgPopup,setShowBgPopup]=useState(false);const[showSyncInfo,setShowSyncInfo]=useState(false);const[syncConflict,setSyncConflict]=useState(null);const[showDesignPopup,setShowDesignPopup]=useState(false);const[showChangelog,setShowChangelog]=useState(false);const[clExpanded,setClExpanded]=useState({});const syncCodeRef=useRef(null);const _cloudDebounce=useRef(null);const _sessionIdRef=useRef(Math.random().toString(36).slice(2,10)+Date.now().toString(36));const[sessionTakeover,setSessionTakeover]=useState(false);const[confirmDisconnect,setConfirmDisconnect]=useState(false);const[usernameInput,setUsernameInput]=useState("");const[showLockInfo,setShowLockInfo]=useState(false);const[confirmLock,setConfirmLock]=useState(false);const[showLockWarning,setShowLockWarning]=useState(false);const[leaderboardData,setLeaderboardData]=useState([]);const[lbSort,setLbSort]=useState("echoes");const _devicesRef=useRef({});
  ...
  const[_popupClosing,_setPopupClosing]=useState(false);const[_wikiClosing,_setWikiClosing]=useState(false);
  const[_launching,_setLaunching]=useState(false);
  const[_enforcerDeath,_setEnforcerDeath]=useState(null);
  const[_metaTabFrom,_setMetaTabFrom]=useState(null);
  const _transTimerRef=useRef(null);
  const _PHASE_ENTRY={metashop:"left",phantom_info:"left",settings:"right",history:"down",hyperecho:"down",leaderboard:"down",practise:"down",playground:"down"};
 ...
 fetch(SUPABASE_URL+"/rest/v1/saves",{method:"POST",headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},body:JSON.stringify({code,data:{meta:ms,history:JSON.parse(h),tut:t,session_id:_sessionIdRef.current,devices:_devs},updated_at:new Date().toISOString()})}).catch(()=>{});}catch(e){}
 },[]);
 const _cloudPull=useCallback(()=>{
 const code=syncCodeRef.current;if(!code||!_SYNC_OK)return;
 fetch(SUPABASE_URL+"/rest/v1/saves?code=eq."+code+"&select=data",{headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY}}).then(r=>r.json()).then(rows=>{if(rows.length>0&&rows[0].data){const cd=rows[0].data;if(cd.devices){_devicesRef.current=cd.devices;if(Object.keys(cd.devices).length>0&&!cd.devices[_sessionIdRef.current]){setSessionTakeover(true);return;}}else if(cd.session_id&&cd.session_id!==_sessionIdRef.current){setSessionTakeover(true);return;}if(cd.meta){const cloudAt=cd.meta.savedAt||0;const localAt=metaRef.current.savedAt||0;if(cloudAt>localAt){setMeta(cd.meta);try{localStorage.setItem("vs4-meta",JSON.stringify(cd.meta));}catch(e){}if(cd.history)try{localStorage.setItem("vs4-history",JSON.stringify(cd.history));}catch(e){}if(cd.tut)try{localStorage.setItem("vs4-tut",cd.tut);}catch(e){}}}}}).catch(()=>{});
 },[]);
 ...
 const fetchLeaderboard=useCallback(()=>{if(!_SYNC_OK)return;fetch(SUPABASE_URL+"/rest/v1/saves?select=code,data",{headers:{"apikey":SUPABASE_ANON_KEY,"Authorization":"Bearer "+SUPABASE_ANON_KEY}}).then(r=>r.json()).then(rows=>{const lb=rows.filter(r=>r.code!=="9292"&&r.data?.meta).map(r=>({username:r.data.meta.username||"Unknown Pilot",echoes:r.data.meta.totalEchoesEarned||0,wave:r.data.meta.highWave||0})).filter(p=>p.echoes>=1);setLeaderboardData(lb);}).catch(()=>{});},[]);
 useEffect(()=>{syncCodeRef.current=syncCode;if(_returnToPauseRef.current){_returnToPauseRef.current=false;setPhase("menu");}if(syncCode)setShowTutPrompt(false);if(syncCode==="9292"){setMeta(prev=>{const nx={...prev};const comp={};LAB_UPGRADES.forEach(lu=>{comp[lu.id]=lu.levels.length;});nx.lab={...(nx.lab||{}),slots:3,completed:comp,active:nx.lab?.active||[],totalWaves:nx.lab?.totalWaves||0,saved:nx.lab?.saved||[]};nx.ownedDesigns=SHIP_DESIGNS.filter(d=>d.cost>0).map(d=>d.id);nx.bossShards=Math.max(nx.bossShards||0,500);saveMeta(nx);return nx;});}},[syncCode]);
 useEffect(()=>{
 if(!_SYNC_OK)return;
 ...
 case"enf_boss":{const _bp=Math.floor(gs.time/500)%4;if(_bp===0){const n=16;for(let i=0;i<n;i++){const ra=(PI2/n)*i+gs.time*0.002;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra)*bs*0.6,vy:Math.sin(ra)*bs*0.6,size:5,dmg:dmg*0.4});}for(let i=0;i<8;i++){const ra2=(PI2/8)*i+gs.time*0.002+0.4;gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(ra2)*bs*1.1,vy:Math.sin(ra2)*bs*1.1,size:3,dmg:dmg*0.3});}}else if(_bp===1){for(let i=-4;i<=4;i++)gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a+i*0.1)*bs*1.3,vy:Math.sin(a+i*0.1)*bs*1.3,size:5,dmg:dmg*0.7});}else if(_bp===2){for(let i=0;i<6;i++){gs.eBullets.push({x:10,y:rand(50,GH-50),vx:bs*0.8,vy:rand(-0.3,0.3)*bs,size:5,dmg:dmg*0.5});gs.eBullets.push({x:GW-10,y:rand(50,GH-50),vx:-bs*0.8,vy:rand(-0.3,0.3)*bs,size:5,dmg:dmg*0.5});}gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*bs*1.8,vy:Math.sin(a)*bs*1.8,size:4,dmg:dmg*0.9});}else{const _la=gs.time*0.004;for(let arm=0;arm<4;arm++){const armA=_la+arm*(PI2/4);for(let j=0;j<6;j++){gs.eBullets.push({x:e.x,y:e.y,vx:Math.cos(armA)*bs*(0.4+j*0.15),vy:Math.sin(armA)*bs*(0.4+j*0.15),size:4,dmg:dmg*0.35});}}}break;}
 default:break;
 }
 ...
 if(e._lassoed){}else if(!e.telegraphing&&!e._sniperLineActive){e.teleTimer-=dt;if(e.teleTimer<=0){e.telegraphing=true;e.aimAngle=ag(e,p);e.teleTimer=700;}}
 ...
 if(e.burnTimer>0){e.burnTimer-=dt;const burnTick=e.burnDmg*dt*0.001;e.hp-=burnTick;trackDmg(gs,"Flame Burn",burnTick);if(e.hp<=0&&!e._burnKilled){e._burnKilled=true;sp(gs,e.x,e.y,e.color||"#ff5544",12,4);sp(gs,e.x,e.y,"#ff6622",8,3);}}
 ...
 ctx.beginPath();ctx.moveTo(p.x,p.y-p.size-4);ctx.lineTo(p.x-p.size,p.y+p.size);ctx.lineTo(p.x,p.y+p.size*0.4);ctx.lineTo(p.x+p.size,p.y+p.size);ctx.closePath();ctx.fill();ctx.shadowBlur=0;const _sd=SHIP_DESIGNS.find(d=>d.id===(metaRef.current.shipDesign||"none"));if(_sd&&_sd.draw){const _dcId=metaRef.current.designColor||metaRef.current.shipColor||"cyan";const _dcObj=SHIP_COLORS.find(c=>c.id===_dcId)||SHIP_COLORS[0];_sd.draw(ctx,p.x,p.y,p.size,_dcObj.color);ctx.globalAlpha=1;}
 ...
 <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,position:"sticky",top:0,zIndex:1,background:"rgba(6,6,14,0.97)",paddingBottom:8}}>
...
<CodexSec id="labs" title={"META LAB UPGRADES"} color="#44ccaa">
 ...
 .vs-title-glow{animation:pulseGlow 4s ease-in-out infinite}
        ...
        @keyframes shipLaunch{from{transform:scale(1) translateY(0)}to{transform:scale(0.22) translateY(440px)}}
 ...
 `}</style> <div style={{position:"relative",width:GW*sc2,height:GH*sc2,outline:meta.showBorder!==false?"1px solid #2a2a3a":"none",border:"none",zIndex:1,overflow:"hidden",background:"#06060e"}}>
 <canvas ref={canvasRef} width={GW} height={GH} style={{width:"100%",height:"100%",display:"block",imageRendering:"pixelated"}} />
 {phase!=="playing"&&<div style={{position:"absolute",inset:0,background:"#06060e",zIndex:5,opacity:phase==="menu"&&(!_transTo||_transTo==="menu")?0:1,transition:"opacity 220ms ease",pointerEvents:"none"}}><SubMenuBG/></div>}
 {showWiki&&<div style={{position:"absolute",inset:0,zIndex:15,animation:_wikiClosing?"transOutRight 200ms cubic-bezier(0.22,1,0.36,1) forwards":"transInRight 200ms cubic-bezier(0.22,1,0.36,1) both"}}>{Wiki()}</div>}
 ...
 <div style={{marginTop:14,zIndex:_launching?20:1,position:"relative",animation:_launching?"shipLaunch 600ms cubic-bezier(0.5,0,0.5,1) forwards":"none"}}><ShipDisplay onClick={_launching?undefined:()=>setShowStats(p=>!p)} size={56} /></div>
 ...
 <button onClick={()=>{if(_launching)return;_setLaunching(true);setTimeout(()=>{_setLaunching(false);initGame();},500);}} style={{...bs2("#00e5ff"),marginTop:8,padding:"12px 40px",fontSize:16,zIndex:1,position:"relative",animation:_launching?"menuFadeOut 400ms ease-out forwards":"none"}} {...hv("#00e5ff")}>LAUNCH</button>
 ...
 <button onClick={()=>goTo("metashop")} style={{...bs2("#bb77ff44"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#bb99ff"}}>META</button>
 <button onClick={()=>goTo("phantom_info")} style={{...bs2("#ff44ff33"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#cc66cc"}}>PHANTOM</button>
 <button onClick={()=>{setEnforcerMode(false);goTo("playground");}} style={{...bs2("#44ccaa33"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#55aa88"}}>PLAYGROUND</button>
 <button onClick={()=>goTo("practise")} style={{...bs2("#cc884433"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#cc9966"}}>PRACTISE</button>
 </div>
 ...
 {<button onClick={()=>{setHeInfoId(null);setAbInfoId(null);setSprintInfo(false);goTo("hyperecho");}} style={{...bs2("#ffcc4433"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#ffcc44"}}>HYPERECHO</button>}
 <button onClick={()=>goTo("settings")} style={{...bs2("#55667744"),padding:"5px 12px",fontSize:10,borderWidth:1,color:"#778899"}}>SETTINGS</button>
 </div>
 {leaderboardData.length>0&&(()=>{const sorted=[...leaderboardData].sort((a,b)=>lbSort==="echoes"?(b.echoes-a.echoes):(b.wave-a.wave));const top3=sorted.slice(0,3);while(top3.length<3)top3.push(null);return <div style={{width:"100%",maxWidth:300,marginTop:14,zIndex:1,position:"relative"}}>
 <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
 <span style={{color:"#556677",fontSize:8,letterSpacing:2}}>LEADERBOARD</span>
 ...
 <span style={{color:lbSort==="echoes"?"#bb77ff":"#44ccaa",fontSize:9,fontFamily:"monospace"}}>{p?(lbSort==="echoes"?"⬢ "+p.echoes.toLocaleString():"W"+p.wave):""}</span>
 </div>)}
 <button onClick={()=>goTo("leaderboard")} style={{width:"100%",marginTop:4,padding:"4px 8px",background:"none",border:"1px solid #33445544",borderRadius:3,cursor:"pointer",fontFamily:"inherit",fontSize:8,color:"#667788",letterSpacing:1}}>VIEW ALL →</button>
 </div>;})()}
 </div>
 )}
 ...
 <h2 style={{color:"#ccddee",fontSize:17,letterSpacing:3,margin:0}}>LEADERBOARD</h2>
 ...
 {[...leaderboardData].sort((a,b)=>lbSort==="echoes"?(b.echoes-a.echoes):(b.wave-a.wave)).map((p,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:i===0?"#1a1408":i%2===0?"#0a0a14":"transparent",borderRadius:3,marginBottom:1,border:i===0?"1px solid #ffcc4422":"1px solid transparent"}}>
 <span style={{color:i===0?"#ffcc44":i===1?"#ccddee":i===2?"#cc8844":"#99aabb",fontSize:10}}><span style={{color:"#556677",fontSize:9,marginRight:6,fontFamily:"monospace"}}>{String(i+1).padStart(2," ")}.</span>{p.username}</span>
 <span style={{color:lbSort==="echoes"?"#bb77ff":"#44ccaa",fontSize:10,fontFamily:"monospace"}}>{lbSort==="echoes"?"⬢ "+p.echoes.toLocaleString():"Wave "+p.wave}</span>
 </div>)}
 {leaderboardData.length===0&&<div style={{color:"#556677",fontSize:10,textAlign:"center",padding:20}}>No players found.</div>}
 </div>
 </div>
 ...
 <span style={{color:"#334455",fontSize:8,letterSpacing:1}}>Version 2.2.2</span>
 ...
 <div style={{color:"#44ccaa",fontSize:13,fontWeight:"bold",letterSpacing:1,marginBottom:8}}>v2.2.2</div>
 ...
 {_enforcerDeath&&(
 <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(6,6,14,0.92)",zIndex:20,animation:"popupBgEnter 250ms ease-out both"}}>
 <div style={{textAlign:"center",animation:"popupEnter 350ms cubic-bezier(0.22,1,0.36,1) both"}}>
 <h2 style={{color:_enforcerDeath.defeated?"#44ccaa":"#ff5577",fontSize:24,letterSpacing:4,margin:0}}>{_enforcerDeath.defeated?"ENFORCER DEFEATED":"DEFEATED"}</h2>
 <div style={{color:"#667788",fontSize:11,marginTop:10,letterSpacing:1}}>{(_enforcerDeath.enemy||"").toUpperCase()}</div>
 <div style={{color:"#aabbcc",fontSize:13,marginTop:14}}>Survived: <span style={{color:_enforcerDeath.defeated?"#44ccaa":"#ccddee"}}>{_enforcerDeath.survived}s</span></div>
 <div style={{display:"flex",gap:10,marginTop:24,justifyContent:"center"}}>
 <button onClick={()=>{const enemy=_enforcerDeath.enemy;_setEnforcerDeath(null);setTimeout(()=>startPlayground(enemy,true),100);}} style={{...bs2("#ff5577"),padding:"8px 22px",fontSize:11}} {...hv("#ff5577")}>RETRY</button>
 <button onClick={()=>{_setEnforcerDeath(null);goTo("menu");}} style={{...bs2("#55667744"),padding:"8px 22px",fontSize:11,borderWidth:1,color:"#8899aa"}}>MENU</button>
 </div>
 </div>
 </div>
 )}
 ...
 <button onClick={()=>goTo("metashop")} style={bs2("#bb77ff")} {...hv("#bb77ff")}>META</button>
 <button onClick={()=>initGame()} style={bs2("#00e5ff")} {...hv("#00e5ff")}>RETRY</button>
 <button onClick={()=>goTo("menu")} style={{...bs2("#55667744"),borderWidth:1,color:"#8899aa",padding:"10px 20px"}}>MENU</button></div>
 </div>
 )}
 ...
 :<button onClick={()=>{(()=>{_setMetaTabFrom(metaTab);setMetaTab("abilities");})();setAbInfoId(null);}} style={{padding:"5px 14px",background:metaTab==="abilities"?"#1a1a2e":"transparent",borderTop:`1px solid ${metaTab==="abilities"?"#44ddcc66":"#33445544"}`,borderLeft:`1px solid ${metaTab==="abilities"?"#44ddcc66":"#33445544"}`,borderRight:`1px solid ${metaTab==="abilities"?"#44ddcc66":"#33445544"}`,borderBottom:"none",borderRadius:"4px 4px 0 0",color:metaTab==="abilities"?"#44ddcc":"#667788",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>Ability Upgrades</button>}
 <button onClick={()=>{(()=>{_setMetaTabFrom(metaTab);setMetaTab("lab");})();setAbInfoId(null);}} style={{padding:"5px 14px",background:metaTab==="lab"?"#1a1a2e":"transparent",borderTop:`1px solid ${metaTab==="lab"?"#ff884466":"#33445544"}`,borderLeft:`1px solid ${metaTab==="lab"?"#ff884466":"#33445544"}`,borderRight:`1px solid ${metaTab==="lab"?"#ff884466":"#33445544"}`,borderBottom:"none",borderRadius:"4px 4px 0 0",color:metaTab==="lab"?"#ff9966":"#667788",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>Lab Upgrades</button>
 </div>
 </div>

 <div key={metaTab} style={{flex:"1 1 0",minHeight:0,display:"flex",flexDirection:"column",animation:(_transStage==="exiting"&&_transFrom==="metashop")?undefined:((()=>{const _tabs=["ship","abilities","lab"];const _ci=_tabs.indexOf(metaTab);const _pi=_tabs.indexOf(_metaTabFrom||metaTab);return _ci>_pi?"tabSlideRight":"tabSlideLeft";})()+" 200ms cubic-bezier(0.22,1,0.36,1) both")}}>
 ...
 const locked=_pHW<lu2.minWave||(lu2.tier===2&&(meta.metaTier||1)<2);
 ...
 <div style={{color:maxed?"#ffcc44":isCurrentSlot?"#44ddcc":"#ccddee",fontSize:12,fontWeight:"bold"}}>{lu2.name} <span style={{color:lu2.tier===2?"#bb99ff":"#667788",fontSize:8}}>{lu2.tier===2?"T2 · ":""}Lv.{cl}/{lu2.levels.length}{locked?(lu2.tier===2&&(meta.metaTier||1)<2?` · Needs Tier 2`:(_pHW<lu2.minWave?` · Needs wave ${lu2.minWave}`:"")):"" }{isCurrentSlot?" · Current":""}</span></div>
